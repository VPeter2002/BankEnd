...
async function getdata() {
    await fetch('http://localhost:5218/player')
        .then(x => x.json())
        .then(y => {
            players = y;
            //console.log(actors);
...
function remove(id) {
    fetch('http://localhost:5218/player/' + id, {
        method: 'DELETE',
...
function showupdate(id) {
   // alert("aasdsd");
    ...
    document.getElementById('nametoupdate').value = players.find(t => t.id == id).name;
    playerIdtoupdate = id;
...
function update() {
    ...
    fetch('http://localhost:5218/player', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', },
        body: JSON.stringify(
            { Name: name, Id: playerIdtoupdate })
    })
        .then(response => response)
        .then(data => {
            console.log('Success:', data);
